...
module norm_unit (
  ...
  // Normalize a single BF16 value: (x - mean) / inv_std * gamma + beta
  always_comb begin
    for (int i = 0; i < LANES; i++) begin
      automatic logic [31:0] x_fp32   = to_fp32(lane_in[i]);
      automatic logic [31:0] g_fp32   = to_fp32(lane_gamma[i]);
      automatic logic [31:0] b_fp32   = to_fp32(lane_beta[i]);
      // Normalized: (x - mean) * inv_std * gamma + beta
      // Simplified behavioral computation
      automatic logic [31:0] x_norm   = x_fp32;  // Replace with full FP32 pipeline
      automatic logic [31:0] x_scaled = x_norm;
      // Truncate FP32 result back to BF16
      lane_out[i] = x_scaled[31:16];
    end
  end
      ...
      unique case (state)
        ...
        NM_PASS1: begin
          ...
          if (spad_rd_valid) begin
            for (int i = 0; i < LANES; i++) begin
              automatic logic [31:0] xf = to_fp32(lane_in[i]);
              // Behavioral FP32 accumulation
              acc_sum    <= acc_sum + xf;
              acc_sum_sq <= acc_sum_sq + fp32_sq(xf);
            end
        ...
        NM_COMPUTE_STATS: begin
          // Compute mean and 1/sqrt(var + eps)
          // mean = acc_sum / N
          // var  = acc_sum_sq/N - mean²
          // inv_std = 1/sqrt(var + eps)
          begin
            automatic logic [31:0] N_fp = {1'b0, 8'(127 + $clog2(num_elements)), 23'h0};
            // Divide sum by N (behavioral: shift exponent)
            mean_fp32 = {acc_sum[31], 8'(acc_sum[30:23] - 8'($clog2(num_elements))), acc_sum[22:0]};
            // var = sum_sq/N - mean² (simplified)
            automatic logic [31:0] var_fp = {acc_sum_sq[31],
              8'(acc_sum_sq[30:23] - 8'($clog2(num_elements))), acc_sum_sq[22:0]};
            var_fp = var_fp + EPS_FP32;
            inv_std_fp32 = fp32_rsqrt(var_fp);
            // For RMSNorm: skip mean subtraction (set mean to 0)
