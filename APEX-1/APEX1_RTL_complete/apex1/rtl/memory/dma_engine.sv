    ...
    if (!rst_n) begin
      ...
      for (int i = 0; i < 8; i++) begin
        unique case (ch_state[i])
          ...
          CH_WAIT: begin
            // Fire when scheduled cycle arrives
            if (global_cycle >= ch_desc[i].fire_cycle)
              ch_state[i] <= ch_desc[i].direction ? CH_WRITEBACK : CH_FETCH;
          end

          CH_FETCH: begin
            // Issue HBM read — handled combinationally below
            // Advance when HBM accepts request
            if (hbm_rd_ready && hbm_rd_valid)
              ch_state[i] <= CH_RECEIVE;
          end

          CH_RECEIVE: begin
            // Write each HBM burst word to SPAD
            if (hbm_rd_data_valid && spad_wr_ack) begin
              ...
              if (hbm_rd_last || ch_bytes_remaining[i] <= (SPAD_DATA_W/8)) begin
                ch_state[i] <= CH_DONE;
              end
            end
          end
          ...
          CH_WRITEBACK: begin
            // SPAD → HBM: read SPAD then write HBM
