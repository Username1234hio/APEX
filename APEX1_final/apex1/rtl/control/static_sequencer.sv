...
module static_sequencer #(
  ...
  logic [2:0] dma_ch_rr;
  logic       unit_done;   // completion signal from whichever unit is active
  always_ff @(posedge clk or negedge rst_n) begin
        ...
        SEQ_ISSUE: begin
          ...
          unique case (instr.opcode)
            ...
            OP_MATMUL, OP_CONV2D, OP_SPARSE_MM: begin
              ...
              mxu_op_code[cluster_rr]  <= opcode_t'(instr.opcode);
              ...
              mxu_dtype[cluster_rr]    <= dtype_t'(instr.dtype);
              mxu_sparse[cluster_rr]   <= (instr.opcode == OP_SPARSE_MM);

              // OPT-3: Double-buffer prefetch.
              // While this tile executes, immediately fire a DMA to prefetch
              // the next tile's A matrix into the alternate SPAD bank.
              // The DMA engine uses a free channel (dma_ch_rr+1 mod 8).
              // The next tile's address is inferred as operand_a + tile_size.
              // In a real compiler this would be explicit; here we approximate.
              if (pc + 1 < instr_total && dma_desc_ready) begin
                dma_desc_valid      <= 1'b1;
                dma_ch_sel          <= dma_ch_rr;
                // Prefetch next tile: advance A address by one tile (dim_m * dim_k * 2B for BF16)
                dma_desc.hbm_addr   <= {instr.operand_b[8:0], instr.operand_a}
                                       + {9'b0, instr.dim_m} * {16'b0, instr.dim_k} * 28'd2;
                // Target alternate SPAD bank (offset by half the SPAD space)
                dma_desc.spad_addr  <= instr.operand_c + (1 << (SPAD_ADDR_W - 1));
                dma_desc.byte_count <= {12'b0, instr.dim_m} * {16'b0, instr.dim_k} * 28'd2;
                dma_desc.fire_cycle <= global_cycle;   // Fire immediately
                dma_desc.direction  <= 1'b0;           // HBM → SPAD
                dma_desc.valid      <= 1'b1;
              end
            end
            ...
            OP_REDUCE_MAX, OP_TRANSPOSE: begin
              sfu_op_valid <= 1'b1;
              sfu_op_code  <= opcode_t'(instr.opcode);
