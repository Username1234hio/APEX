...
module systolic_array #(
  ...
  // ── Output capture ────────────────────────────────────────────
  // result_valid fires one cycle after drain_en.
  // Caller must wait ROWS+COLS-2 cycles after the last activation row
  // before asserting drain_en, to allow the systolic pipeline to drain.
  // OPT-1: With BF16_PIPE_STGS=1 in mac_cell, the BF16 path has one extra
  // cycle of latency. The drain timing in the harness uses ROWS+COLS which
  // already includes this margin. For INT8/FP8 (combinational multipliers)
  // timing is unchanged.
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      result_valid <= 1'b0;
